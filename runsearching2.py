...
def error_occured():
    template = os.path.join(TEMPLATES_DIR, "error_occured.png")
    ...
    _, max_val, _, max_loc = cv2.minMaxLoc(result)
    if max_val >= 0.8:
        template = os.path.join(TEMPLATES_DIR, "click_ok.png")
        resize = upscale(template)
        result = cv2.matchTemplate(screenshot_cv, resize, cv2.TM_CCOEFF_NORMED)
        _, max_val, _, max_loc = cv2.minMaxLoc(result)
        if max_val >= 0.8:
            tx, ty = max_loc
            th, tw = resize.shape[:2]
            x=tx + tw // 2
            y=ty + th // 2
            pyautogui.click(x,y)
...
def get_to_final_step(cx, cy, max_steps=15):
    probe_x=cx
    probe_y=cy
    ...
    if not px is None or not  py is None:
        logger.info(f"[ACTION] Clicking ({px},{py})")
        error_occured()
        click_twice(px, py)
        time.sleep(3)
...
def capture_view_play(x, y,crop_h=45, threshold=0.6, right_margin=40, template_name="play_button.png"):
    screen_w, screen_h = pyautogui.size()
